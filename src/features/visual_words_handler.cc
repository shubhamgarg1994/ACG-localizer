...
bool visual_words_handler::k_nn_search_flann_uchar(std::vector<unsigned char> &descriptors, uint32_t nb_descriptors, std::vector<uint32_t> &assignments, std::vector<float> &distances)
{
	...
	std::cout << "Size of mFlann" << sizeof(mFlannAssignmentsKNN.data[0]) / sizeof(int) << std::endl;
	std::cout << "Size of mFlann" << sizeof(mFlannAssignmentsKNN.data[1]) / sizeof(int) << std::endl;
	std::cout << "SIze of descriptors" << nb_descriptors << std::endl;
	for (uint32_t i = 0; i < nb_descriptors; ++i)
	{
